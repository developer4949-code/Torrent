...
const ACTIVE_STATUSES: JobStatus[] = ['PENDING', 'SCHEDULED', 'PICKED', 'RUNNING'];
...
function App() {
  ...
  const fetchJobs = useCallback(async (): Promise<boolean> => {
    try {
      const [jobsRes, metricsRes] = await Promise.all([
        ...
        fetch('http://localhost:8080/api/cluster/metrics').catch(() => null)
      ]);
  ...
  const queueCustomJob = (e: FormEvent) => {
    ...
    const timeout = Math.max(1, Math.floor(Number(customJob.timeoutSeconds) || 60));
    queueJob({
      jobType: customJob.jobType.trim(),
      ...
      <div className="app-container">
        ...
        {connStatus === 'offline' && (
          <div className="offline-banner" role="alert">
            ...
            <div>
              ...
              <code className="mono">http://localhost:8080</code> but cannot connect. Start the
        ...
        <section className="stats-grid reveal-on-scroll">
          ...
          <div className="stat-card">
            ...
            <div>
              ...
              <div className="stat-trend">Messages waiting: {kafkaLag}</div>
            </div>
          </div>
          <div className="stat-card">
            ...
            <div>
              <div className="stat-value">{stagedJobs.length}</div>
        ...
        <section className="glass-panel architecture-section reveal-on-scroll" style={{ transitionDelay: '0.1s' }}>
          ...
          <div className="arch-flow">
            ...
            <div className={`arch-node node-worker ${activeStates.worker ? 'is-active' : ''}`}>
              <Server size={22} className="node-icon" />
              <h3>Worker Nodes</h3>
